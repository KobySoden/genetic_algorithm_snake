...
class SnakeModel:
    ...
    def Nump_input(self):
        a = np.asarray(self.food_state, dtype='float32')
        a = a.flatten(order ='C')
        b = np.asarray(self.snake_state, dtype='float32')
        b = b.flatten(order = 'C')
        a = np.append(a,b)
...
class Snakebrain(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.model.add(Dense(20, input_dim=204, activation='relu'))
...
class controller:
    def __init__(self):
        ...
        self.step_time_millis = 200
        self.init_pop_size = 1000
        self.generation_size = 100
        self.step_limit = 100
        self.genetic = Genetic_alg(self.init_pop_size, self.generation_size, .5, .2)
        self.brain_index = 0
